...
# Make sure you import the database calls from your updated "discord_db.py"
...
class DraftView(View):
    ...
    async def button_callback(self, interaction: Interaction):
        ...
        if interaction.data['custom_id'] == 'end_draft':
            ...
            if self.giving_up_draft:
                ...
                await self.txt_channel.send(
                    f"You gave up being drafter, pick a new drafter from your team", 
                    delete_after=10
                )
            else:
                self._set_buttons()
    ...
    def _get_players_from_db(self) -> List[Any]:
        ...
        for p in self.players:
            ...
            player = execute_function_single_row_return('get_player', str(p.id))
            # Set a fallback name
...
def _add_stats_to_player(player):
    ...
    if games_played['played'] == 0:
        ...
        rank_data = execute_function_single_row_return('get_player_rank', player['id'])
