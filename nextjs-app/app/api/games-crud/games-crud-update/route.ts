...
export async function PUT(req: NextRequest) {
    ...
    if (!id || team_won === undefined || !status) {
        return NextResponse.json({ error: "Missing game id, team_won, or status" }, { status: 400 });
    }
    ...
    try {
        ...
        const actualGame: { status: string } | undefined = await new Promise((resolve, reject) => {
            db.get(
        ...
        if (actualGame.status !== status) {
            db.close();
            return NextResponse.json({ error: "Game status mismatch. Possible tampering detected." }, { status: 403 });
        }

        // Get all players from gamePlayers where game_id = id
        const players: Array<{ player_id: number; team: number }> = await new Promise((resolve, reject) => {
            db.all(
            ...
            players.map(({ player_id, team }) => {
                return new Promise((resolve, reject) => {
                    db.run(
                        ...
                        (err) => {
                            if (err) {
                                console.error(`Error updating MMR for player ${player_id}:`, err);
                                return reject(err);
                            }
                            resolve(null);
                        }
        ...
        await new Promise((resolve, reject) => {
            db.run(
                `UPDATE Game SET status = 'OVER' WHERE id = ?`,
                [id],
                (err) => {
                    if (err) {
        ...
        db.close();
        return NextResponse.json({ success: true, message: "MMR updated and game marked as OVER" });
